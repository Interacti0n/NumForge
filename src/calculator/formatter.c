...
/* Apply application limits to the public numeric formatter. */
                                          ...
                                          char **result)
{
    ...
    if (context == NULL) return CALCULATOR_NULL_ARGUMENT;
    ...
    if (context->output_scale > CALCULATOR_MAX_OUTPUT_SCALE) status = CALCULATOR_VALUE_TOO_LARGE;
    else status = calculator_from_bigdecimal_status(bigdecimal_format(value, context->output_scale, context->rounding, result));
