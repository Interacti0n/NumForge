...
static BigIntStatus integer_root(BigInt *root, const BigInt *number, uint32_t degree)
{
    ...
    if (bits <= degree) return bigint_set_string(root, "1");

    BigInt *power = bigint_create(), *next = bigint_create();
    BigInt *order = bigint_create(), *previous_order = bigint_create();
    BigIntStatus status = BIGINT_OUT_OF_MEMORY;
    char text[16];
    if (power == NULL || next == NULL || order == NULL || previous_order == NULL) goto cleanup;
#define INTEGER_TRY(operation) do { status = (operation); if (status != BIGINT_OK) goto cleanup; } while (0)
    ...
    for (;;)
    {
        ...
        if (bigint_compare(next, root) >= 0) break;
        INTEGER_TRY(bigint_copy(root, next));
    }
cleanup:
    bigint_destroy(power); bigint_destroy(next);
    bigint_destroy(order); bigint_destroy(previous_order);
    return status;
#undef INTEGER_TRY
}
...
{
    if (status == BIGINT_OK) return BIGDECIMAL_OK;
    if (status == BIGINT_OUT_OF_MEMORY) return BIGDECIMAL_OUT_OF_MEMORY;
    if (status == BIGINT_VALUE_TOO_LARGE) return BIGDECIMAL_VALUE_TOO_LARGE;
...
static BigDecimalStatus set_root(BigDecimal *result, const BigInt *root,
    bool negative, int64_t scale, bool sticky)
{
    char *coefficient = bigint_to_string(root);
    if (coefficient == NULL) return BIGDECIMAL_OUT_OF_MEMORY;
    size_t capacity = strlen(coefficient) + 32U;
    char *text = numforge_malloc(capacity);
    BigDecimalStatus status = BIGDECIMAL_OUT_OF_MEMORY;
    if (text != NULL)
    {
        (void)snprintf(text, capacity, "%s%s%sE%" PRId64,
            negative ? "-" : "", coefficient, sticky ? "1" : "", -scale);
        status = bigdecimal_set_string(result, text);
    }
    free(text); free(coefficient);
    return status;
}
    ...
    uint32_t degree, int64_t digits, BigDecimalRoundingMode rounding)
{
    ...
    if (degree == 1 || bigint_is_zero(value->coefficient)) return bigdecimal_copy(result, value);

    BigInt *number = bigint_create(), *root = bigint_create();
    BigInt *power = bigint_create(), *order = bigint_create();
    BigDecimal *temporary = bigdecimal_create();
    char *coefficient = NULL, *scaled = NULL;
    BigDecimalStatus status = BIGDECIMAL_OUT_OF_MEMORY;
    if (number == NULL || root == NULL || power == NULL || order == NULL || temporary == NULL) goto cleanup;
#define ROOT_TRY(operation) do { status = (operation); if (status != BIGDECIMAL_OK) goto cleanup; } while (0)
    ...
    if (coefficient == NULL) { status = BIGDECIMAL_OUT_OF_MEMORY; goto cleanup; }
    size_t length = strlen(coefficient);
    /* floor((length - 1 - scale) / degree), without overflowing at INT64_MIN. */
    ...
    if (scaled == NULL) { status = BIGDECIMAL_OUT_OF_MEMORY; goto cleanup; }
    ...
    ROOT_TRY(decimal_status(integer_root(root, number, degree)));
    /* The exact-finite branch above failed: the omitted tail is nonzero.
...
cleanup:
    free(coefficient); free(scaled);
    bigint_destroy(number); bigint_destroy(root);
    bigint_destroy(power); bigint_destroy(order);
