...
static BigDecimalStatus bigdecimal_normalize(BigDecimal *value)
{
    ...
    status = bigdecimal_from_bigint_status(bigint_strip_decimal_zeros(value->coefficient, &removed));
    ...
    {
        if (!bigdecimal_i64_sub(value->scale, INT64_MAX, &value->scale)) return BIGDECIMAL_SCALE_OVERFLOW;
        removed -= INT64_MAX;
    }
    return bigdecimal_i64_sub(value->scale, (int64_t)removed, &value->scale)
        ? BIGDECIMAL_OK : BIGDECIMAL_SCALE_OVERFLOW;
}
...
/* A reduced denominator has a finite decimal expansion exactly when its
    ...
    const BigDecimal *b, int64_t digits, BigDecimalRoundingMode rounding)
{
    ...
    if (result == NULL || a == NULL || b == NULL) return BIGDECIMAL_NULL_ARGUMENT;
    if (digits < 1 || !bigdecimal_valid_rounding(rounding)) return BIGDECIMAL_INVALID_ARGUMENT;
    ...
    status = bigdecimal_from_bigint_status(bigint_gcd(factor, a->coefficient, b->coefficient));
    if (status == BIGDECIMAL_OK)
    ...
    for (size_t index = 0; index < 2U && status == BIGDECIMAL_OK; index++)
    {
        ...
        while (status == BIGDECIMAL_OK && !bigint_is_one(denominator))
        {
            ...
            status = bigdecimal_from_bigint_status(bigint_div_mod(quotient, remainder, denominator, factor));
            if (status != BIGDECIMAL_OK || !bigint_is_zero(remainder)) break;
    ...
    if (status != BIGDECIMAL_OK) goto cleanup;
    ...
    const BigDecimal *b, int64_t digits, BigDecimalRoundingMode rounding)
{
    ...
    free(a_text);
    free(b_text);
    if (!bigdecimal_i64_sub(digits - 1, exponent, &target)) return BIGDECIMAL_SCALE_OVERFLOW;

    /* Divide coefficients, then apply the external scales. This avoids huge
     * powers of ten when the ratio has a tiny/huge but compact magnitude. */
...
)
{
    ...
    if (!(bigdecimal_i64_add(target_scale, b->scale, &exponent) &&
        ...
        !(bigdecimal_i64_sub(target_scale, a->scale, &exponent) &&
          bigdecimal_i64_add(exponent, b->scale, &exponent))) return BIGDECIMAL_SCALE_OVERFLOW;
